...
ppKron mat = tagT $ case (sing :: SPrimePower pp) of
  (SPP (STuple2 sp _)) -> do
    ...
    let d = (valuePPow @pp) `div` withWitness valuePrime' sp
...
extIndicesCRT =
  ...
  in U.generate phi' $ fromIndexPair tots . swap . (`divMod` (phi' `div` phi))
...
extIndicesCoeffs =
  let (_, phi, phi', tots) = indexInfo @m @m'
  in V.generate (phi' `div` phi) $ \i1 ->
       U.generate phi (\i0 -> fromIndexPair tots (i1,i0))
